...
substitute_modes = ["AGIR","DGIR", "attention_weight",'AGIR'] 
data_dirs = ["sci-fi_comedy", "comedy_drama", "action_comedy", "sci-fi_thriller"]
...
for sub_mode, data_dir in zip(substitute_modes,data_dirs):
    ...
    Path(f"./RQ1/substitution_ratio_res/").mkdir(parents=True, exist_ok=True)
    res_df.to_csv(f"./RQ1/substitution_ratio_res/{data_dir}.csv")